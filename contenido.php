<?php 

	function contenido(){
		$datos=array();

		$datos[0]="img/better.jpg"."||".
				  "Better Call Saul"."||".
				  "Una serie nominada al Emmy que deriva de 'Breaking Bad' y sigue la metamorfosis del abogado Jimmy McGill en Saul Goodman, el asesor inescrupuloso de Walter White.";

		$datos[1]="img/boys.jpg"."||".
				  "The boys"."||".
				  "La serie tiene lugar en un mundo en el que los superhéroes representan el lado oscuro de la celebridad y la fama. Un grupo de vigilantes que se hacen llamar 'The Boys' decide hacer todo lo posible por frenar a los superhéroes que están perjudicando a la sociedad, independientemente de los riesgos que ello con lleva.
";

		$datos[2]="img/breaking-bad.jpg"."||".
				  "Breaking Bad"."||".
				  "Un profesor de Química de secundaria con cáncer terminal se asocia a un exestudiante para asegurar el futuro de su familia al fabricar y vender metanfetamina.";

		$datos[3]="img/club.jpg"."||".
				  "El club de la pelea"."||".
				  "Un joven sin ilusiones lucha contra su insomnio, consecuencia quizás de su hastío por su gris y rutinaria vida. En un viaje en avión conoce a Tyler Durden, un carismático vendedor de jabón que sostiene una filosofía muy particular: el perfeccionismo es cosa de gentes débiles; en cambio, la autodestrucción es lo único que hace que realmente la vida merezca la pena. Ambos deciden entonces formar un club secreto de lucha donde descargar sus frustaciones y su ira que tendrá un éxito arrollador.";

		$datos[4]="img/cobrakai.png"."||".
				  "Cobra Kai"."||".
				  "Décadas después del torneo que les cambió la vida, se reaviva la rivalidad entre Johnny y Daniel. Esta secuela continúa la historia de las películas de Karate Kid.";

		$datos[5]="img/gotham.jpg"."||".
				  "Gotham"."||".
				  "Mucho antes de ser el comisionado, James Gordon ya se enfrentaba al crimen y a la corrupción de Gotham para vengar la muerte de los padres de Bruce Wayne.";

		$datos[6]="img/mindhunter.jpg"."||".
				  "Mindhunter"."||".
				  "Dos agentes del FBI que buscan desarrollar la ciencia criminal analizan la psicología de atroces asesinatos mediante encuentros peligrosamente cercanos con los homicidas.";	
				  
		$datos[7]="img/origen.jpg"."||".
				  "El Origen"."||".
				  "En este alucinante thriller de ciencia ficción, un hombre a cargo de un negocio de espionaje entra al subconsciente de sus objetivos para moldear sus pensamientos.";	  

		$datos[8]="img/viejaguardia.jpg"."||".
				  "La vieja Guardia"."||".
				  "Cuatro guerreros inmortales que llevan siglos protegiendo a la humanidad son perseguidos por sus misteriosos poderes, justo cuando descubren a una nueva integrante.";	

		$datos[9]="img/vikingos.jpg"."||".
				  "Vikingos"."||".
				  "Este drama realista recuenta las hazañas del vikingo Ragnar Lothbrok durante la expansión nórdica al retar a un líder falto de visión.";


		return $datos;
	}

 ?>